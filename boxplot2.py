...
name = cwd + '\\classificationdataZ' + '.csv'
...
for dataname in measureddatalist:
    ...
    for j in range(0, len(qrange) - 1):
        if qlistdupe[j] not in qlistdupe[0:j]:
            ...
            pos = qlist.index(qlistdupe[j], 0, j)
    ...
    # plt.show()
    plt.title('Box plot for ' + dataname + ' measurements of each type of question of all subjects')

    plt.savefig(os.path.join(cwd + '\IMGBox', 'boxplot' + dataname.replace(' ', '_') + ".jpg"))
    plt.close()