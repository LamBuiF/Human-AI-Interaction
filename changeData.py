...
for i in subjectslist:
    ...
    for j in range(0, len(qnum)):
        if math.isnan(qnum[j]) or math.isnan(acceptdeny[j]) or (confidence[j] <= 3):
            delrow.append(j)
    ddf = ddf.drop(delrow)
    # print(ddf)
    ...
    print(df)
    print(df2)
    df.to_csv('lineardata3_' + str(i).zfill(3) + '.csv', encoding='utf-8', index=False)
    df2.to_csv('classificationdata3_' + str(i).zfill(3) + '.csv', encoding='utf-8', index=False)
